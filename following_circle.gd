...
func _process(_delta: float) -> void:
	...
	var a = get_global_mouse_position().x - global_position.x
	var b = get_global_mouse_position().y - global_position.y
	if (b == 0): b = 0.0001
	if (a == 0): a = 0.0001
	...
	var is_top:bool = get_global_mouse_position().y < global_position.y
	if (c > max): c = max;
